...
@implementation XYAdButton
- (instancetype)initWithSkipType:(SkipType)skipType{
    self = [super init];
    if (self) {
        _skipType = skipType;
        CGFloat y = IPHONE_X ? 44 : 20;
        //环形
        if(skipType == SkipTypeRoundTime || skipType == SkipTypeRoundText || skipType == SkipTypeRoundProgressTime || skipType == SkipTypeRoundProgressText){
            self.frame = CGRectMake(XY_ScreenW-55,y, 42, 42);
        }else{//方形
            self.frame = CGRectMake(XY_ScreenW-80,y, 70, 35);
        }
        switch (skipType) {
            case SkipTypeNone:{
                self.hidden = YES;
...
-(void)startRoundDispathTimerWithDuration:(CGFloat )duration{
    ...
    dispatch_source_set_event_handler(_roundTimer, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.qroundLayer.strokeStart += 1/(duration/period);
